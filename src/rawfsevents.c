...
{
  pthread_mutex_destroy(&fsevents->lock);
  pthread_cond_destroy(&fsevents->init);
  fsevents->thread = NULL;
  fsevents->loop = NULL;
}
...
{
  fse_loop_t *fsevents = (fse_loop_t *)data;
...
{
  fse_loop_t *fsevents = watcher->global;
  pthread_mutex_lock(&fsevents->lock);
  if (!fsevents->loop)
  {
    pthread_create(&fsevents->thread, NULL, fse_run_loop, fsevents);
    while (!fsevents->loop)
    {
